...
- (void)viewDidLoad
{
    ...
    _inputImage.contentMode = UIViewContentModeScaleAspectFit;
    _inputImage.clipsToBounds = YES;
    [_inputImage setImage: self.origImage];
    
    _inputAge.text = self.age;
    
    _agedAge.text = [self clustertoage: self.cluster];
    
    [self loadAgedImage];
    [self loadCroppedImage];
    [self loadNonBlendImage];
    
    [_inputImage setUserInteractionEnabled:YES];
    [_cropInput setUserInteractionEnabled:YES];
    [_outImage setUserInteractionEnabled:YES];
    [_outNonBlendImage setUserInteractionEnabled:YES];
    
    UITapGestureRecognizer *tapper0 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(sel0:)];
    UITapGestureRecognizer *tapper1 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(sel1:)];
    UITapGestureRecognizer *tapper2 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(sel2:)];
    UITapGestureRecognizer *tapper3 = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(sel3:)];
    
    [_inputImage addGestureRecognizer:tapper0];
    [_cropInput addGestureRecognizer:tapper1];
    [_outImage addGestureRecognizer:tapper2];
    [_outNonBlendImage addGestureRecognizer:tapper3];
   
    
    //_inputImage.userInteractionEnabled = YES;
   // _inputImage.contentMode = UIViewContentModeCenter;
    
   // _twoFinger = [[UIPinchGestureRecognizer alloc] initWithTarget:self action:@selector(twoFinger:)];
   // [_inputImage addGestureRecognizer:_twoFinger];
